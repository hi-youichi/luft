    ...
    let pmap_fn = lua.create_function(move |lua, (items, map_fn): (Table, Function)| {
        ...
        let outcome: mlua::Result<Table> = (|| {
            ...
            let (tx, mut rx) = mpsc::channel::<(
                usize,
                std::result::Result<maestro_core::contract::backend::AgentResult, SchedulerError>,
            )>(count.max(1) * 4);
...
fn dispatch_task(
    ...
    sched: &std::sync::Arc<maestro_core::Scheduler>,
    handle: &tokio::runtime::Handle,
    journal: &Option<std::sync::Arc<maestro_core::journal::JournalStore>>,
    run_id: maestro_core::contract::ids::RunId,
) {
...
mod tests {
    use super::register;
    use maestro_core::contract::backend::RunContext;
