...
async fn route(method: &str, path: &str, base_dir: &Path) -> (&'static str, &'static str, Vec<u8>) {
    ...
    match path {
        ...
        "/api/runs" => match luft::query::list_runs(base_dir) {
            Ok(runs) => {
...
async fn handle_run_api(path: &str, base_dir: &Path) -> (&'static str, &'static str, Vec<u8>) {
    ...
    match suffix {
        "" => match luft::query::get_status(&run_dir, base_dir) {
            Ok(Some(status)) => {
        ...
        "events" => match luft::query::get_events(&run_dir, base_dir) {
            Ok(events) => {
        ...
        "report" => match luft::query::get_report(&run_dir, base_dir) {
            Ok(luft::query::ReportStatus::Found(value)) => {
                let body = serde_json::json!({"found": true, "value": value});
                let json = serde_json::to_vec(&body).unwrap_or_else(|_| b"{}".to_vec());
                ("200 OK", "application/json", json)
            }
            Ok(luft::query::ReportStatus::RunFinished) => (
                "200 OK",
                "application/json",
                serde_json::json!({"found": false, "run_finished": true}).to_string().into_bytes(),
            ),
            Ok(luft::query::ReportStatus::NotFound) => (
                "200 OK",
                "application/json",
