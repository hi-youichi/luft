...
/// `luft run --nl "<task>"`, the planner sends this text + the user's task
...
/// ```
///
/// The LLM acts as a "compiler" from natural language to the Luft Lua DSL.
...
/// ## Keeping in sync
///
/// Every primitive documented here MUST be registered in [`luft_runtime::sandbox`]
