...
use maestro_core::state::{CheckpointStatus, RunCheckpoint};
use maestro_planner::PlanMeta;
...
fn collect_running_agents(
    events: &[AgentEvent],
    completed: &HashMap<maestro_core::contract::ids::AgentId, maestro_core::state::AgentResultCache>,
) -> HashMap<u32, Vec<AgentRow>> {
    let mut done_agents: HashSet<maestro_core::contract::ids::AgentId> = HashSet::new();
    for e in events {
...
mod tests {
    use super::*;
    use maestro_core::contract::ids::{AgentId, PhaseId, RunId, TokenUsage};
    use maestro_core::state::{AgentResultCache, PhaseSummary};
    use maestro_planner::{MetaPhase, PlanMeta};
    use chrono::TimeZone;
            ...
            checkpoint.agent_results.insert(
                ...
                AgentResultCache {
                    ...
                    findings: (0..findings_count)
                        .map(|i| maestro_core::contract::finding::Finding {
                            kind: "test".into(),
                            severity: maestro_core::contract::finding::Severity::Info,
    ...
    fn header_elapsed_from_run_events() {
        ...
        let events = vec![
            ...
            AgentEvent::RunDone {
                run_id: checkpoint.run_id,
                status: maestro_core::contract::event::RunStatus::Completed,
