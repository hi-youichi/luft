...
pub struct ResolvedScript {
    pub script: String,
    pub meta: Option<luft_planner::PlanMeta>,
}
...
pub struct RunSpec {
    ...
    pub workflow_meta: Option<luft_planner::PlanMeta>,
}
...
mod tests {
    ...
    use luft_runtime::{ExecLimits, Runtime};
    ...
    #[test]
    fn slug_sources_luft_workflow() {
        let spec = RunSpec {
            run_id: RunId::now_v7(),
            run_dir_name: String::new(),
            script: String::new(),
            task_label: "luft workflow".to_string(),
            resuming: false,
            extra_args: serde_json::json!({}),
            workflow_meta: None,
        };
    ...
    fn assign_dir_name_luft_label() {
        ...
        assert!(!spec.run_dir_name.is_empty());
        assert!(
            spec.run_dir_name.starts_with("luft-workflow_"),
            "expected slug prefix 'luft-workflow_', got '{}'",
            spec.run_dir_name
        );
    }
    ...
    async fn resolve_script_script_variant() {
        ...
        let result = resolve_script(
            ...
            backend,
            luft_planner::PlannerConfig::default(),
        )
        .await
        .unwrap();
    ...
    async fn resolve_fresh_script_variant() {
        ...
        assert_eq!(spec.task_label, "luft workflow");
    ...
    fn resume_check_not_found() {
        let temp_dir = std::env::temp_dir().join("luft_svc_test_resume_notfound");
