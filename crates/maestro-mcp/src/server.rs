...
/// The MCP server: owns the Maestro instance and run registry.
...
impl McpServer {
    /// Create a new server with the given Maestro instance.
    ///
    ...
    pub async fn dispatch_method(
        ...
        match method {
            ...
            "tools/call" => Ok(handle_call(params, &self.maestro, &self.runs, &self.search_dirs).await),
...
mod tests {
    ...
    async fn dispatch_initialize() {
        ...
        assert_eq!(result["serverInfo"]["name"], "maestro");
    }
    ...
    async fn dispatch_message_initialize() {
        ...
        assert_eq!(parsed["result"]["serverInfo"]["name"], "maestro");
    }
    ...
    async fn search_dirs_override() {
        ...
        let maestro = maestro::Maestro::builder()
            ...
            .build()
            .unwrap();
        let server = McpServer::new(maestro).search_dirs(vec![dir.path().to_path_buf()]);
