...
//! - `get_run_status` — query run status via `maestro-service`
//! - `get_run_events` — query run event log

use maestro::Maestro;
use maestro_core::contract::event::AgentEvent;
use maestro_runtime::validate_workflow;
...
pub async fn handle_call(
    ...
    match name {
        Some("execute_workflow") => execute_workflow(maestro, runs, &arguments).await,
        Some("list_workflows") => list_workflows_tool(search_dirs),
        Some("get_run_status") => get_run_status_tool(maestro, runs, &arguments).await,
        Some("get_run_events") => get_run_events_tool(maestro, runs, &arguments).await,
...
pub async fn execute_workflow(
    ...
    let handle = match maestro.start_script(&script).await {
        Ok(h) => h,
        Err(e) => {
...
pub async fn get_run_events_tool(
    ...
    let events = match maestro.events(&run_dir) {
        Ok(e) => e,
        Err(e) => {
...
mod tests {
    ...
    fn filter_events_since_found_returns_after() {
        use maestro_core::contract::event::RunStatus;
        let events = vec![
            AgentEvent::RunDone {
    ...
    async fn handle_call_unknown_tool() {
        let maestro = build_test_maestro();
        let runs = new_run_registry();
        let dirs = vec![];

        let params = json!({ "name": "bogus", "arguments": {} });
        let result = handle_call(&params, &maestro, &runs, &dirs).await;
    ...
    async fn get_run_status_missing_run_id() {
        ...
        let runs = new_run_registry();
        let args = json!({});
        let result = get_run_status_tool(&maestro, &runs, &args).await;
        assert_eq!(result["isError"], true);
    ...
    async fn get_run_events_missing_run_id() {
        ...
        let runs = new_run_registry();
        let args = json!({});
        let result = get_run_events_tool(&maestro, &runs, &args).await;
        assert_eq!(result["isError"], true);
        assert!(result["content"][0]["text"].as_str().unwrap().contains("missing required"));
    }
    ...
    async fn execute_workflow_missing_args() {
        ...
        let runs = new_run_registry();
        let args = json!({});
        let result = execute_workflow(&maestro, &runs, &args).await;
        assert_eq!(result["isError"], true);
    ...
    fn event_matches_id_with_matching_content() {
        ...
        let event = AgentEvent::RunDone {
            ...
            status: RunStatus::Completed,
            run_id: uuid::Uuid::nil(),
            total_tokens: maestro_core::contract::ids::TokenUsage::default(),
            ts: chrono::Utc::now(),
        };
    ...
    async fn execute_then_get_status_and_events() {
        ...
        let status_result = get_run_status_tool(&maestro, &runs, &status_args).await;
        ...
        let events_result = get_run_events_tool(&maestro, &runs, &events_args).await;
