...
//! resolved as: explicit `--log-file` > `~/.luft/logs/luft.log`. Parent
...
/// is supplied: `$HOME/.luft/logs/luft.log`. Used as a fallback inside
/// [`init`] when `log_file` is `None`.
pub fn default_log_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".luft").join("logs").join("luft.log"))
}
...
mod tests {
    ...
    fn temp_log(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("luft_log_{}", uuid::Uuid::now_v7()));
    ...
    fn init_creates_parent_directories() {
        let _lock = LOG_LOCK.lock().unwrap();
        let base = std::env::temp_dir().join(format!("luft_log_nested_{}", uuid::Uuid::now_v7()));
    ...
    fn default_log_path_uses_home() {
        // Verify the resolver puts the file under $HOME/.luft/logs/.
        ...
        assert!(s.contains(".luft"), "expected .luft in {s}");
        assert!(s.ends_with("luft.log"), "expected luft.log in {s}");
    }
