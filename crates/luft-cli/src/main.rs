//! `luft` binary entry point.
//!
//! CLI commands:
//! - `luft run "<NL>"` — NL → Lua via planner, then execute
//! - `luft run /<wf>` — Run workflow.lua file
//! - `luft run --resume` — Resume from checkpoint
//! - `luft run --confirm` — Show script for confirmation (default: auto-approve)
//!
...
struct Cli {
    ...
    /// Write program-log (tracing) to this file. Default: `~/.luft/logs/luft.log`.
