...
/// Runs are stored in `.luft/runs` relative to the current working directory.
pub(crate) fn runs_base_dir() -> std::path::PathBuf {
    std::path::PathBuf::from(".").join(".luft").join("runs")
}
...
mod tests {
    ...
    #[test]
    #[serial_test::serial]
    fn runs_base_dir_immediate_parent_is_dot_luft() {
        let p = runs_base_dir();
        let parent = p
            .parent()
    ...
    #[test]
    #[serial_test::serial]
    fn runs_base_dir_components_chain_through_dot_then_dot_luft() {
        let p = runs_base_dir();
        ...
        assert_eq!(
            comps,
            vec![".".to_string(), ".luft".to_string(), "runs".to_string(),],
    ...
    fn runs_base_dir_is_relative_and_not_rooted() {
        ...
        assert_eq!(p, Path::new(".").join(".luft").join("runs"));
    }

    #[test]
    #[serial_test::serial]
    fn runs_base_dir_equals_manually_constructed_path() {
        let expected: PathBuf = PathBuf::from(".").join(".luft").join("runs");
