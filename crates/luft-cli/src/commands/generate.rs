...
async fn generate_script_with_backend(
    ...
    let cfg = luft::planner::PlannerConfig {
        planner_model,
    ...
    let planned = luft::planner::plan_workflow(&args.nl, backend, &cfg).await?;
...
mod tests {
    use super::*;
    use crate::GenerateArgs;
    use luft::core::{MockBackend, MockBehavior, TokenUsage};
