...
fn resolve_backend_id(args: &RunArgs) -> Result<String> {
    ...
    if is_nl && backend_id == "mockfile" {
        anyhow::bail!(
            "mockfile backend requires --workflow. Use `luft generate --with-mock` first."
        );
    }
...
fn load_model_and_planner(args: &RunArgs) -> (Option<String>, luft::planner::PlannerConfig) {
    ...
    let planner_cfg = luft::planner::PlannerConfig {
        planner_model,
        ..Default::default()
    };
    (model, planner_cfg)
...
async fn resolve_run_spec(
    ...
    backend_id: &str,
    model: Option<String>,
    planner_cfg: luft::planner::PlannerConfig,
    base_dir: &Path,
...
fn create_run_backend(
    ...
    if backend_id == "mockfile" {
        ...
        if !mock_path.exists() {
            anyhow::bail!(
                "mock file not found: {}. Run `luft generate --with-mock` first.",
                mock_path.display()
            );
        }
...
fn spawn_printer_task(
    ...
    use luft::core::contract::event::ProgressDelta;
...
async fn try_fix_script(
    ...
    use luft::core::contract::backend::AgentTask;
    use luft::core::contract::ids::AgentId;
    use luft::core::RunContext;
    ...
    if result.status != luft::core::contract::backend::AgentStatus::Ok {
...
mod tests {
    ...
    use luft::core::contract::event::{ProgressDelta, RunStatus};
    use luft::core::contract::ids::{RunId, TokenUsage};
    ...
    fn write_report_flat_filename_empty_parent() {
        let id = RunId::now_v7();
        let name = format!("__luft_test_write_report_{id}.tmp");
