...
impl SkillManager {
    ...
    pub fn new() -> Result<Self> {
        ...
        if !source_dir.exists() {
            return Err(crate::install::types::InstallError::SkillSourceNotFound(source_dir));
        }
    ...
    pub fn install_for_agents(&self, agents: &[AgentType]) -> Result<Vec<BridgeInstallResult>> {
        let target_dirs = self.get_target_directories(agents)?;
...
mod tests {
    ...
    fn test_install_for_codex() {
        ...
        let skill_manager = SkillManager::new().unwrap();
        let results = skill_manager.install_for_agents(&[AgentType::Codex]).unwrap();
    ...
    fn test_install_for_claude() {
        ...
        let skill_manager = SkillManager::new().unwrap();
        let results = skill_manager.install_for_agents(&[AgentType::Claude]).unwrap();
    ...
    fn test_install_for_multiple_agents() {
        ...
        let skill_manager = SkillManager::new().unwrap();
        let results = skill_manager.install_for_agents(&[AgentType::Codex, AgentType::Claude]).unwrap();
