...
> **状态**: 📝 设计阶段（2025-08-19）
> **目标**: 新增 `maestro-mcp` crate，作为外部 MCP server 供 AI 工具（Claude Code / Cline 等）调用，提供 workflow 编写指南（resources）和执行/查询能力（tools）。

---

## 1. 背景

Maestro 当前有两套 MCP 实现，分别服务不同场景：

| 实现 | 位置 | 用途 | 协议方向 |
|------|------|------|---------|
| `mcp_server.rs` | `maestro-cli/src/commands/` | structured output 验证（opencode 子进程） | agent → maestro |
| `mcp.rs` | `maestro/src/` | agent 上报 findings/artifacts/logs | agent → maestro |

两者都是 **数据面**（agent 向 maestro 上报），缺少 **控制面**（外部工具向 maestro 下发指令）。
...
│  │                                                   │  │
│  │  内部调用 maestro facade / service / runtime      │  │
│  └───────────────────────────────────────────────────┘  │
│                                                         │
│  ── Workflow 执行时（ACP 桥接） ──────────────────────  │
│                                                         │
...
| | 现有 `mcp.rs` / `mcp_server.rs` | 本设计 `maestro-mcp` |
|---|---|---|
| 方向 | agent → maestro（数据面上报） | client → maestro（控制面指令） |
| 消费者 | workflow 内部的 agent 子进程 | 外部 AI 工具 |
| 启动方式 | maestro 内部自动拉起 | `maestro mcp serve` 手动启动 |
| 代码共享 | 不共享，独立实现 | — |
...
```toml
# crates/maestro-mcp/Cargo.toml
[dependencies]
maestro        = { path = "../maestro" }          # facade API (run_script / run_workflow)
maestro-service   = { path = "../maestro-service" }   # query (status / events)
maestro-runtime   = { path = "../maestro-runtime" }   # validate_workflow
maestro-core      = { path = "../maestro-core" }      # types (RunId / RunOutcome / ...)
...
```
maestro-cli ──► maestro-mcp ──► maestro (facade)
                              ──► maestro-service
                              ──► maestro-runtime
                              ──► maestro-core
```
...
内容来源：编译时 `include_str!("../../maestro-planner/src/lua_dsl_reference.md")` 嵌入二进制。
...
`description` 从每个 `.lua` 文件的 `meta.reasoning` 字段提取（使用 `maestro-planner` 的 `extract_meta`）。
  ...
  "resources": [
    {
      ...
      "description": "Complete Lua DSL syntax for writing Maestro workflows"
    },
    {
...
```json
{
  ...
  "description": "Execute a Maestro workflow. Accepts either inline Lua script or a path to a .lua file. Returns immediately with a run_id — use get_run_status to poll progress.",
  "inputSchema": {
    "type": "object",
    "properties": {
...
**前置校验**：执行前先调用 `maestro_runtime::sandbox::validate_workflow(&script)` 进行三层校验（语法 → 结构 → schema）。校验失败则不执行，直接返回错误：
...
**实现**：先 `validate_workflow`，通过后调用 `maestro::Maestro::run_script(script)` 或 `run_workflow(path)`，在 tokio runtime 的 `spawn` 中异步执行，主线程立即返回。
...
**返回**（通过 `maestro-service` query 模块 + `PhasesView`）：
...
```rust
// crates/maestro-mcp/src/server.rs

pub fn run_mcp_server(maestro: Maestro) -> Result<()> {
        ...
        match msg.method.as_deref() {
            ...
            "tools/call"              => {
                ...
                let result = rt.handle().block_on(
                    tools::handle_call(&msg.params, &maestro, &runs)
                );
            }
...
```rust
async fn execute_workflow(maestro: &Maestro, script: &str, args: Value) -> Result<Value> {
    let handle = maestro.run_script(script).await?;  // 非阻塞 spawn
...
`get_run_status` / `get_run_events` 通过 `maestro-service` 的 query 模块查询，从 `RunHandle` 或 storage reader 获取。
...
- [ ] 创建 `crates/maestro-mcp/` crate
- [ ] 实现 `protocol.rs`（JSON-RPC types）
- [ ] 实现 `server.rs`（stdio 主循环）
- [ ] 实现 `resources.rs`（schema + examples + example）
- [ ] CLI 集成 `maestro mcp serve`

### Phase 2: Tools
- [ ] 实现 `tools.rs`（4 个 tool handlers）
- [ ] `execute_workflow` 前置 `validate_workflow` + 接入 `maestro::Maestro` facade
- [ ] `get_run_status` / `get_run_events` 接入 `maestro-service` query
...
| # | 问题 | 当前倾向 | 待确认 |
|---|------|---------|--------|
| 1 | `Maestro` facade 是否已提供异步非阻塞的 `run_script` 接口？ | 需验证 `RunHandle` 是否 spawn 后立即返回 | 查看 facade API |
| 2 | `maestro-service` 的 query 模块是否支持按 `run_id` 查询状态和事件？ | 需验证 query API 完整性 | 查看 service API |
