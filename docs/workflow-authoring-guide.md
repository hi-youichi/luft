...
Luft 采用 **Claude Code Dynamic Workflows** 范式：用户给出自然语言任务，planner LLM 充当"编译器"，将 NL 编译为一段 Lua 编排脚本，runtime 再确定性执行该脚本。
...
Luft 在 Lua 沙箱中注册了以下原语（Lua 全局变量）。完整 API 签名见 [`sdk-reference.md`](./sdk-reference.md)，此处聚焦 **使用场景和最佳实践**。
