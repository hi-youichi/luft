...
cargo run -- run -w examples/hello.lua -b mock \
    --log .maestro/example_logs/hello.jsonl --log-format jsonl
```
...
cargo run -- run -w examples/parallel-demo.lua -b mock \
    --log .maestro/example_logs/parallel.jsonl --log-format jsonl
```
...
cargo run -- run -w examples/pipeline-demo.lua -b mock \
    --log .maestro/example_logs/pipeline.jsonl --log-format jsonl
```
...
cargo run -- run -w examples/converge-demo.lua -b mock \
    --log .maestro/example_logs/converge-mock.jsonl --log-format jsonl
```
...
cargo run -- run -w examples/converge-demo.lua -b opencode \
    --log .maestro/example_logs/converge-opencode.jsonl --log-format jsonl
```
...
cargo run -- run -w examples/deep-research.lua -b opencode \
    -o deep-research.md \
    --log .maestro/example_logs/deep-research.jsonl --log-format jsonl
```
...
cargo run -- run -w examples/architecture-report.lua -b opencode \
    -o architecture.md \
    --log .maestro/example_logs/architecture.jsonl --log-format jsonl
```
...
{"type":"agent_started","prompt_preview":"You are exploring the Maestro...",...}  ← 探测 agent 枚举模块
...
{"type":"report_emitted","report":{"markdown":"# Maestro 架构概览报告\n..."}}
{"type":"run_done","status":"Completed",...}
```
