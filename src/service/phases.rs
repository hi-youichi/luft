...
pub fn build_phases_view(checkpoint: &RunCheckpoint, events: &[AgentEvent]) -> PhasesView {
    ...
    let phases = match &checkpoint.workflow_meta {
        Some(meta) => build_from_meta(meta, checkpoint, events),
...
fn collect_running_agents(
    events: &[AgentEvent],
    completed: &HashMap<crate::core::contract::ids::AgentId, crate::core::state::AgentResultCache>,
) -> HashMap<u32, Vec<AgentRow>> {
    let mut done_agents: HashSet<crate::core::contract::ids::AgentId> = HashSet::new();
    for e in events {
...
mod tests {
    ...
    use crate::planner::{MetaPhase, PlanMeta};
    use chrono::TimeZone;
            ...
            checkpoint.agent_results.insert(
                ...
                AgentResultCache {
                    ...
                    findings: (0..findings_count)
                        .map(|i| crate::core::contract::finding::Finding {
                            kind: "test".into(),
                            severity: crate::core::contract::finding::Severity::Info,
    ...
    fn meta_phase_elapsed_from_ts() {
        ...
        let events = vec![
            AgentEvent::PhaseStarted {
                run_id: checkpoint.run_id, phase_id: 1, label: "p".into(), planned: 1, ts: ts(100),
            },
    ...
    fn fallback_events_reconstructs_phases() {
        ...
        let events = vec![
            AgentEvent::PhaseStarted {
                run_id: checkpoint.run_id, phase_id: 1, label: "discover".into(), planned: 2, ts: ts(100),
            },
            AgentEvent::PhaseStarted {
                run_id: checkpoint.run_id, phase_id: 2, label: "report".into(), planned: 1, ts: ts(200),
            },
        ];
        let view = build_phases_view(&checkpoint, &events);
