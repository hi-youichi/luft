...
use maestro::service::phases::{PhasesView, PhaseStatus};
use anyhow::Result;
use std::io::Write;
...
pub(crate) fn phases_cmd_inner(
    ...
    let checkpoint: maestro::core::state::RunCheckpoint = serde_json::from_str(&checkpoint_content)
...
fn render_phases(w: &mut impl Write, view: &PhasesView) -> std::io::Result<()> {
    ...
    let source_note = match view.source {
        maestro::service::phases::PhasesSource::Meta => "meta",
        maestro::service::phases::PhasesSource::EventsFallback => "events fallback",
    };
    if total == 0 {
...
mod tests {
    ...
    use maestro::planner::{MetaPhase, PlanMeta};
    ...
    fn seed_run_with_meta(meta: PlanMeta, current_phase: u32) -> String {
        ...
        store.init_run_with_meta(id, "test task", meta).unwrap();
